...
class Akun extends CI_Controller {
	...
	public function user($filter1='', $filter2='', $filter3='')
	{
		if ($this->session->userdata('log_in') == TRUE){
			$where_admin['username'] 	= $this->session->userdata('username');
			$data['admin'] 				= $this->ADM->get_admin('',$where_admin);
			if ($data['admin']->user_role === 'admin' || $data['admin']->user_role === 'kepegum') {
				...
				if ($data['action'] == 'view'){
					...
					$data['jml_data']			= $this->ADM->count_all_admin('', $like_admins);
						...
						$where_admins['username'] = $data['admin']->username;
						$like_admins[$data['cari']]	= $data['q'];
						$data['jml_data']			= $this->ADM->count_all_admin($where_admins, $like_admins);
					}
				...
				if($simpan){
					if(do_hash($data['password_recent'], 'md5') == $data['admin']->password) {
					   if($data['password'] == $data['password_ulang']) {
						   ...
						   $where_edit2['pegawai_nip']	= $data['username'];
						   if($data['password'] <> '') {
							   $edit['password']		= do_hash(($data['password']), 'md5'); 
							   $edit2['pegawai_password']		= do_hash(($data['password']), 'md5'); 
						   }
						   $this->ADM->update_admin($where_edit, $edit);
						   $jml_pengguna				= $this->ADM->count_all_pegawai($where_edit2);
						   if($jml_pengguna > 0) {
